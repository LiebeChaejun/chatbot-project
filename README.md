...
## 기술 선택 (Tech Decisions)

프로젝트에서 사용한 기술을 단순히 나열하기보다, 각 기술이 어떤 역할을 수행하며 어떤 문제를 해결하기 위해 선택되었는지 중심으로 정리했습니다.

### 🤖 AI Workflow

| 기술 | 선택 이유 |
|------|-----------|
| **LangGraph** | 질문 분류 → 질문 재구성 → 검색 → 답변 생성으로 이어지는 멀티 에이전트 워크플로우를 명확하게 관리하기 위해 선택했습니다. 또한 Checkpointer를 활용해 대화 상태를 안정적으로 유지했습니다. |
| **LangChain** | 문서 로딩, 텍스트 분할, Retriever, 프롬프트 체인 등 RAG 파이프라인을 일관된 인터페이스로 구성하기 위해 사용했습니다. |
| **OpenAI GPT-4o-mini** | 작업의 특성에 맞게 LLM을 분리했습니다. 의도 분류·질문 재구성에는 `temperature=0`, 답변 생성에는 `temperature=0.7`을 적용하여 일관성과 자연스러움을 모두 확보했습니다. |
| **OpenAI text-embedding-3-small** | 초기에는 HuggingFace 로컬 임베딩 모델(`BAAI/bge-m3`)을 사용했지만, Render 무료 인스턴스의 메모리 제한(OOM)을 해결하기 위해 API 기반 임베딩으로 전환했습니다. 이를 통해 Docker 이미지 크기와 서버 시작 속도도 함께 개선했습니다. |
| **FAISS** | 임베딩된 매뉴얼 문서를 빠르게 검색하기 위한 벡터 검색 엔진으로 사용했습니다. |
...
| 기술 | 선택 이유 |
|------|-----------|
| **FastAPI** | 비동기 기반 API 서버를 구축하기 위해 선택했습니다. `lifespan`을 활용하여 LangGraph와 SQLite 리소스를 서버 시작 시 한 번만 초기화하도록 구성해 요청마다 반복 초기화되는 비용을 줄였습니다. |
| **AsyncSqliteSaver** | LangGraph의 Checkpointer를 활용하여 각 대화(Thread)의 메시지 이력을 영속적으로 저장하기 위해 사용했습니다. |
| **SQLite (aiosqlite)** | 대화 메타데이터(제목, 소유자, 생성일 등)를 별도로 관리하여 메시지 이력과 책임을 분리하고 유지보수성을 높였습니다. |
...
| 기술 | 선택 이유 |
|------|-----------|
| **React + TypeScript** | 컴포넌트 기반 UI와 타입 안정성을 확보하기 위해 사용했습니다. API 응답은 Discriminated Union으로 모델링하여 성공/실패 응답을 컴파일 단계에서 안전하게 처리하도록 설계했습니다. |
| **Vite** | 빠른 개발 서버와 간결한 설정을 통해 개발 생산성을 높이기 위해 선택했습니다. |
| **Tailwind CSS** | 반복되는 UI를 빠르게 구현하면서도 컴포넌트 단위로 스타일을 관리하기 위해 사용했습니다. |
...
| 기술 | 선택 이유 |
|------|-----------|
| **Docker** | 로컬과 배포 환경의 의존성 차이를 줄이기 위해 프로젝트 초기부터 도입했습니다. 실제로 컨테이너 빌드 과정에서 `sentence-transformers` 의존성 누락을 발견하여 배포 전에 수정할 수 있었습니다. |
| **Render** | Docker 기반으로 백엔드를 배포했습니다. 무료 인스턴스의 메모리 제약을 고려하여 임베딩 전략을 개선하는 등 실제 운영 환경에서 발생하는 문제를 해결했습니다. |
| **Vercel** | React 애플리케이션을 손쉽게 배포하고, 환경변수를 통해 백엔드와 연동하기 위해 사용했습니다. |
