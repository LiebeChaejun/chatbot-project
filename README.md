...
###  1. 이전 대화를 기억하고 자연스럽게 이어갑니다.

"그거 환불 조건은?"처럼 이전 질문을 참조하는 표현도 LangGraph 기반 질문 재구성을 통해 독립적인 검색 쿼리로 변환한 뒤 문맥에 맞는 답변을 제공합니다.

#### 구현
- 질문 재구성(Rephrase) 에이전트
- LangGraph Checkpointer를 통한 대화 상태 관리

### 2. 필요한 경우에만 메뉴얼을 검색합니다.

질문의 의도를 먼저 판단하여

- 단순 인나 잡담은 검색 없이 자연스럽게 응답
- 서비스 관련 질문은 RAG 검색 후 답변

불필요한 검색을 줄이고 자연스러운 대화 흐름을 유지했씁니다.
...
검색 결과가 없는 경우 답을 만들어내지 않고 "죄송하지만 문의하신 내용에 대해서는 아는 바가 없습니다."와 같이 정직하게 응답하도록 설계했습니다. 이를 통해 Hallucination을 줄였습니다.

### 4. 여러 상담을 주제별로 동시에 관리할 수 있습니다.
...
### 5. 로그인 없이도 사용자별 대화를 안전하게 분리합니다.
브라우저마다 owner_id를 발급하여 다른 사용자의 대화가 섞이지 않도록 설계했습니다. 
로그인을 구현하지 않다도 실제 배포 환경에서 여러 사용자가 동시에 사용할 수 있습니다.

### 6. 대화에 집중할 수 있는 인터페이스를 제공합니다.
사이드바는 필요할 때만 열 수 있도록 설계했습니다. 대화 화면을 가리지 않으면서도 이전 대화로 빠르게 이동할 수 있습니다.
